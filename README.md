...
- **历史复盘/回测**（复盘XX/回测十年/**分析某年-某年**/当时/上市以来）→ 后复权（`--adjust backward`，历史口径固定）
- **当前/近期/现状判断**（是否状态A/突破/入场参考价/近期会不会有机会/近一两年 /**分析 XX 股票 / XX 怎么样** 这类没带时点词但想看现状的问法）→ 前复权（`--adjust forward`，与行情软件一致）
- **拿不准/无明确历史意图 → 一律前复权**；只有明确出现「历史/复盘/回测/年份区间」才用后复权
